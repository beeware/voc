...
class PythonFormatter {
    ...
    private org.python.types.Str _format() {
        ...
        for (int i = 0; fmt.length > i; i++) {
            ...
            // advance to the next format specification
            if (fmt[i] != '%') {
                outputStringBuilder.append(fmt[i]);
                continue;
            }
            ...
            if (fmt[i] == '(') {
                ...
                for (int openBrackets = 1; openBrackets > 0; i++) {
                    ...
                    if (fmt[i] == '(') {
                        openBrackets++;
                    } else if (fmt[i] == ')' && openBrackets == 1) {
                        openBrackets--;
                    } else {
                        keyBuilder.append(fmt[i]);
                    }
                }
            ...
            long precision = PRECSION_NOT_SET;
            if (fmt[i] == '.') {
                ++i;
                if (fmt[i] == '*') {
                    ++i;
            ...
            if (fmt[i] == 'l' || fmt[i] == 'L' || fmt[i] == 'h') {
                i++;
            }

            /* Finally determine the conversion. It is important to note here
            ...
            java.lang.String preFormattedObject = null;
            switch (conversionChar) {
                ...
                case 'g': case 'G':
                    ...
                    if (value >= java.lang.Math.pow(10, precision) || value < .0001) {
                        ...
                        preFormattedObject = toFixed(fmtObjectJava, precision);
                    }
                    break;
                ...
                case 'c':
                    preFormattedObject = java.lang.String.format(
                        "%" + conversionChar,
                        (java.lang.Character) fmtObjectJava);
                    break;

                case 'i': case 'u': case 'd': case 'o': case 'x': case 'X':
                    // For integer arguments if precision is given it determines zero padding.
                ...
                default:
                    throw new org.python.exceptions.ValueError(
                        String.format("ValueError unsupported format character '%c' (%x) at index %d", fmt[i], (int) fmt[i], i));
            }

            java.lang.String signToUse = determineSignToUse(sign);
            ...
            if (conversionFlags.get('-')) {
                outputStringBuilder.append(signToUse);
                outputStringBuilder.append(alternateIntegerFormPrefix);
                outputStringBuilder.append(preFormattedObject);
                outputStringBuilder.append(padding);
            } else {
                outputStringBuilder.append(padding);
                outputStringBuilder.append(signToUse);
                outputStringBuilder.append(alternateIntegerFormPrefix);
     ...
     */
    private String determineSignToUse(char sign) {
        if (sign == SIGN_POSITIV) {
            if (conversionFlags.get(' ')) {
                return " ";
            } else if (conversionFlags.get('+')) {
                return "+";
            } else {
                return "";
            }
     ...
     */
    private java.lang.String convertFloatingPointTypeByJavaFormatter(
            java.lang.Object num, long precision, char conversionChar) {
     ...
     * @return           The desired String correctly formatted by Java.
     */
    private java.lang.String toFixed(java.lang.Object num, long precision) {
        return convertFloatingPointTypeByJavaFormatter(num, precision, 'f');
    }
     ...
     * @return                  The desired String correctly formatted by Java.
     */
    private java.lang.String toExp(java.lang.Object num, long precision, char conversionChar) {
        return convertFloatingPointTypeByJavaFormatter(num, precision, conversionChar);
    }
     ...
     * @return                The desired String correctly formatted by Java.
     */
    private java.lang.String toInteger(java.lang.Object num, long precision, char conversionChar) {
        return java.lang.String.format("%"
    ...
    private final java.util.Map<java.lang.Character, java.lang.Boolean> conversionFlags;
